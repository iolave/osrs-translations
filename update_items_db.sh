...
echo "[info] retrieving items from osrsbox-db"
json_items=$(curl -s https://www.osrsbox.com/osrsbox-db/items-summary.json | jq -r '
	map(to_entries)
	| flatten
	| group_by(.key)
...
echo "[info] adding type and text to items"
